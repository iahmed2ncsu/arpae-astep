...
import module9_final_packaging
from module9_final_packaging import *
...
tab1, tab2,tab3, tab4,tab5 = st.tabs(["Inputs 1-3", "Inputs 4-9","Run Economic Assessment","Results-Levelized Cost",
...
#        config_input['ODmax_mod4'] = None
    ...
    if 'simulator_option' not in st.session_state:
         st.session_state.simulator_option = None
    simulator_option = st.selectbox(
         label = 'Choose an option for the train simulator', options = ('default', 'force-run','custom'),key = 501)
    if st.button("Submit Selections", key = 502):
        st.session_state.simulator_option = simulator_option
    if st.session_state.simulator_option  == 'default':
        st.write("Table below shows default proportions for coal and petrolium reductions. Click and enter value(s) if\
                 you need to change")
        # tech_frac_df = st.experimental_data_editor(tech_frac_default, key = 503)
        ...
        if sum(abs(sum_dist -1) > tolerance ) != 0:
            ...
            st.button("Submit Selections", key = 503)
    ...
    elif st.session_state.simulator_option == "force-run":
        st.write("The train simulator will run using pre-stored data")
    elif st.session_state.simulator_option == "custom":
        st.write("Upload train and alignment data in appropriate formats. For more info about the formats, please check the user manual")
...
with tab3:
    ...
    if st.button("▶️ Run"):
        ...
        for year in years:
            ...
            # end = time.time()
            # st.write(f"Module 3 Runtime = {round(end-start,2)} seconds")
            ...
            # end = time.time()
            # st.write(f"Module 4 Runtime = {round(end-start,2)} seconds")
            ...
            module5_single_train_simulator.create_energy_intensity_matrix_sts(st.session_state.simulator_option,st.session_state.uploaded_alignment_data, st.session_state.uploaded_train_data)
            end = time.time()
            ...
            module6_calculate_energy_use.calculate_energy_use(str(year), simulator_type="sts")  
            end = time.time()
            st.write(f"Module 6 Runtime = {round(end-start,2)} seconds")
            ...
            # end = time.time()
            # st.write(f"Module 7 Runtime = {round(end-start,2)} seconds")
            ...
            st.write(st.session_state.discount_rate)
            st.write(st.session_state.tech_lifetime)
            st.write(st.session_state.contingency_factor)
            ...
            st.write("Preparing the tarball of the Simulation Results")
            module9_final_packaging.tarball()
