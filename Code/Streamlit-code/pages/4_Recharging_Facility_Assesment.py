...
tab1, tab2, tab3, tab4, tab5 = st.tabs(["About", "Train Schedule","Demand Profile", "Battery Tenders", "Hydrogen Tenders"])
# %% About page

page_title = "A-STEP: Achieving Sustainable Train Energy Pathways"
body = "A-STEP: Achieving Sustainable Train Energy Pathways"
subhead = "Recharging Facility Assessment"\
...
with tab1:
    ...
    st.markdown(
    """This tool focuses on estimating the quantity and dimensions of components required at refueling and recharging facilities so they can serve as intermediary energy transfer points between the energy sources, like the power grid, and the train tenders and locomotives (batteries and hydrogen fuel cells). 

For both battery and hydrogen recharging facilities, simulation is employed. Trains arrive and deliver depleted tenders; they then depart and leave with ones that are recharged. The facility processes the arrivals to prepare for the departures; and the temporal variations in those arrival and departure patterns; as well as considerations of peak load pricing by the utilities; determines how large the facility needs to be; and thus, the operational and capital costs. Each analysis produces results for a specific facility size; and a series of analyses produces a database of configuration results for facilities of various sizes.

The main input is the target recharging capacity per year (e.g., 90 GWh/year). This is portrayed in the form of tenders per year (say 36,000), of a given capacity (e.g., 5 MWh), which are fully or partially depleted (say, on average 50%), which arrive in trains (e.g., in 7200 trains, or about 20 trains per day, with 5 tenders per train). Using Monte Carlo simulation, the train arrivals (and the departures) are then distributed across the 8,760 hours of the year based on inputs about the distributions (in percentages) of train arrivals by month, day of the week, and hour of the day. The pattern of train departures is input separately, but the number of tenders per train and the total number of train departures is held constant so there is no difference between input and output. 
Within the facility, arriving tenders are recharged. Once recharged, tenders are placed in inventory and become part of the supply for satisfying subsequent train departures. A “ready reserve is assumed to exist in case the normal supply of tenders is exhausted.""")
